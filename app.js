import { CATALOG_ITEMS, DOWNLOAD_SOFTWARE_ITEMS, FIRMWARE_GUIDES, FIRMWARE_ITEMS, PACK_BRANDS, SOFTWARE_ITEMS } from "./catalog-data.js";
...
function storePage() {
  ...
  return header("loja", true) +
    "<main id='conteudo' class='store-page'><section class='store-hero'><div class='container'>" + (!STORE_LISTED ? "<span class='unlisted-badge'>Prévia não listada</span>" : "") + "<span class='eyebrow'>Loja-curadoria</span><h1>Equipamentos para tocar, controlar e criar.</h1><p>Reunimos equipamentos relevantes e ofertas selecionadas para você comparar menos e tocar mais.</p><aside class='store-trust-strip' aria-label='Como funciona nossa curadoria'><div class='store-trust-title'><span>i</span><div><strong>Curadoria independente</strong><small>Não somos a fabricante.</small></div></div><ul><li>Preço pesquisado</li><li>Links de parceiros</li><li>Sem custo extra</li></ul><details><summary>Como funciona</summary><p>Selecionamos produtos e acompanhamos oportunidades na Amazon e no Mercado Livre. Quando uma compra é feita por nossos links, podemos receber uma comissão — você não paga nada a mais. Antes de comprar, confirme vendedor, frete, estoque e a revisão exata do produto.</p></details></aside></div></section>" +
...
function firmwareGuideMarkup(guide) {
  ...
  return "<article class='firmware-tutorial'><header class='firmware-tutorial-head'><div><span class='kicker text-blue'>" + escapeHtml(guide.family) + "</span><h2>" + escapeHtml(guide.name) + "</h2><p>" + escapeHtml(guide.method) + "</p></div><span class='firmware-status'>" + escapeHtml(guide.status) + "</span></header><div class='firmware-model-warning'><span>!</span><p><strong>Antes de baixar:</strong> " + escapeHtml(guide.warning) + "</p></div><section class='firmware-prepare'><span class='eyebrow'>" + escapeHtml(guide.prepareLabel || "Separe antes de começar") + "</span><ul>" + guide.before.map(function(item) { return "<li>" + escapeHtml(item) + "</li>"; }).join("") + "</ul>" + downloads + "</section><section class='firmware-tutorial-steps'><div class='section-heading'><div><span class='eyebrow'>Passo a passo</span><h2>Siga nesta ordem.</h2></div><p>" + escapeHtml(guide.stepsIntro || "Reserve alguns minutos e não interrompa a alimentação ou a conexão durante a gravação do firmware.") + "</p></div><ol>" + guide.steps.map(function(step, index) { return "<li><span>" + String(index + 1).padStart(2, "0") + "</span><div><h3>" + escapeHtml(step[0]) + "</h3><p>" + escapeHtml(step[1]) + "</p></div></li>"; }).join("") + "</ol></section><aside class='firmware-recovery'><span>Se algo não sair como esperado</span><p>" + escapeHtml(guide.recovery) + "</p><a href='/suporte/?q=" + encodeURIComponent("a atualização do " + guide.name + " falhou") + "'>Abrir ajuda para este modelo →</a></aside><footer class='firmware-sources'><p>Procedimento revisado em " + escapeHtml(guide.reviewed || "13/08/2026") + " a partir do centro oficial de downloads e da documentação do produto. A fabricante pode alterar versões e etapas.</p><a href='" + guide.official + "' target='_blank' rel='noopener'>Conferir a fonte oficial ↗</a></footer></article>";
}
...
function downloadHubPage() {
  ...
  }).join("") + "<a class='download-hub-card download-hub-software' href='/downloads/software/'><div class='download-card-top'><span class='download-card-code'>APP</span><span>6 ferramentas oficiais · Windows e macOS</span></div><div class='download-card-wave' aria-hidden='true'><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div><div class='download-card-copy'><p>Software M-VAVE</p><h2>Editores e utilitários para configurar seus equipamentos.</h2></div><div class='download-card-action'><span>Acessar softwares</span>" + iconArrow() + "</div></a>";
  return header("", true) +
    "<main id='conteudo' class='downloads-page'><section class='downloads-hero downloads-hub-hero'><div class='container downloads-hero-grid'><div class='downloads-hero-copy'><span class='eyebrow'>Biblioteca digital · acesso reservado</span><h1>Seu som.<br><span class='display-accent'>Bem organizado.</span></h1><p>Uma central simples para baixar a coleção de IRs, chegar direto ao modelo ou instalar o software certo para seu equipamento.</p><div class='download-hero-pills'><span>108 arquivos de IR</span><span>Windows e macOS</span><span>3 instrumentos</span></div></div><div class='download-system-card' aria-hidden='true'><div class='download-system-head'><span><i></i><i></i><i></i></span><strong>MVAVE / LIBRARY</strong><small>ONLINE</small></div><div class='download-system-body'><div><span>01</span><strong>GUITARRA</strong><small>67 FAMÍLIAS</small></div><div><span>02</span><strong>BAIXO</strong><small>24 FAMÍLIAS</small></div><div><span>03</span><strong>VIOLÃO</strong><small>13 FAMÍLIAS</small></div><div><span>04</span><strong>SOFTWARE</strong><small>6 FERRAMENTAS</small></div></div><div class='download-system-foot'><span>WAV</span><span>ZIP</span><span>DMG</span><strong>READY ↓</strong></div></div></div></section><section class='download-hub-section'><div class='container'><div class='download-private-badge'><span>ACESSO POR LINK</span><p>Esta área não aparece no menu ou no sitemap e foi configurada para não ser indexada.</p></div><div class='download-hub-heading'><div><span class='eyebrow'>Escolha sua biblioteca</span><h2>O que você quer baixar?</h2></div><p>Abra o pack de IR correspondente ou a área de softwares oficiais.</p></div><div class='download-hub-grid'>" + cards + "</div></div></section></main>" + footer();
}

function softwarePlatformRow(platform, download) {
  const external = Boolean(download.url);
  const href = external ? download.url : downloadFileUrl(download.arquivo);
  const size = download.tamanho_bytes ? " · " + formatDownloadSize(download.tamanho_bytes) : "";
  const label = download.format === "Mac App Store" ? "Abrir na App Store" : "Baixar " + platform;
  return "<div class='download-software-platform'><div><strong>" + escapeHtml(platform) + "</strong><small>" + escapeHtml(download.format) + size + "</small>" + (download.note ? "<small>" + escapeHtml(download.note) + "</small>" : "") + "</div><a class='btn btn-dark' href='" + escapeHtml(href) + "'" + (external ? " target='_blank' rel='noopener'" : " download") + ">" + escapeHtml(label) + iconArrow() + "</a></div>";
}
...
function render() {
  ...
  else if (route === "downloads" && parts[1] === "software") html = downloadSoftwarePage();
